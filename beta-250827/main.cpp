...
int a[15][15],f,wx,wy,hh,ohh,mode,time1,time2,time3=30,PID,s1=1,bkcolor,textcolor,linecolor,timecolor,bkci=1,tci=0,lci=0,timeci=0,colorn=10,colorl[]={0,WHITE,RGB(165,42,42),RGB(255,140,0),RGB(255,215,0),RGB(46,139,87),RGB(0,191,255),RGB(190,190,190),BROWN,RGB(180,82,205)},checkPattern(int x,int y,int player);
char c[30];
...
void start(),sfill(int t),drawb(int lx,int rx,int y,string str),refresh(),judges1(),runexe(),kill();
int main(){
    codde:
    start();
    code:
    ...
    while(true){
        ...
        if(L_MOUSE){
            ...
            if(p.x>=50&&p.x<650&&p.y>=150&&p.y<750&&hh==0){
                ...
                if(MessageBox(GetHWnd(),"确定要退出吗？","退出",MB_OKCANCEL|MB_ICONQUESTION)==1){
                    goto codde;
                }
...
void start(){
    ...
    while(true){
        ...
        if(p.y>=150&&p.y<=250&&p.x>=270&&p.x<=610){
            sfill(1);
            setfillcolor((linecolor!=WHITE?linecolor+1:linecolor-1));
            floodfill(225,200,linecolor);
            floodfill(655,200,linecolor);
            ttt=1;
        }else if(p.y>=300&&p.y<=400&&p.x>=270&&p.x<=610){
            sfill(2);
            setfillcolor((linecolor!=WHITE?linecolor+1:linecolor-1));
            floodfill(225,350,linecolor);
            floodfill(655,350,linecolor);
            ttt=2;
        }else if(p.y>=450&&p.y<=550&&p.x>=270&&p.x<=610){
            sfill(3);
            setfillcolor((linecolor!=WHITE?linecolor+1:linecolor-1));
            floodfill(230,500,linecolor);
            floodfill(650,500,linecolor);
            ttt=3;
        }else if(p.y>=600&&p.y<=700&&p.x>=270&&p.x<=610){
            sfill(4);
            setfillcolor((linecolor!=WHITE?linecolor+1:linecolor-1));
            floodfill(225,650,linecolor);
            floodfill(655,650,linecolor);
            ttt=4;
        }
        if(ttt==1&&!(p.y>=150&&p.y<=250&&p.x>=270&&p.x<=610)){
            ...
            setfillcolor(bkcolor);
            floodfill(225,500,linecolor);
            floodfill(655,500,linecolor);
            ttt=0;
        ...
        if(KEY_DOWN(L_MOUSE)){
            ...
            if(p.y>=150&&p.y<=250&&p.x>=270&&p.x<=610){
                ...
                outtextxy(50,280,"显示游戏提示");
                line(50,330,850,330);
                judges1();
                outtextxy(50,340,"游戏回合时间");
                line(50,390,850,390);
                rectangle(760,345,840,385);
                outtextxy(50,400,"倒计时进度条颜色");
                line(50,450,850,450);
                setfillcolor(timecolor);
                fillrectangle(800,405,840,445);
                ...
                while(true){
                    if(KEY_DOWN(L_MOUSE)){
                        ...
                        }else if(p.x>=760&&p.x<=840&&p.y>=285&&p.y<=325){
                            s1=!s1;
                            judges1();
                        ...
                        }else if(p.x>=800&&p.x<=840&&p.y>=405&&p.y<=445){
                            if(InputBox(c,10,"请输入倒计时进度条颜色的编号：\n\n黑：0\t\t白：1\n红：2\t\t橙：3\n黄：4\t\t绿：5\n蓝：6\t\t灰：7\n棕：8\t\t紫：9","倒计时进度条颜色",NULL,0,0,0)){
                                t=atoi(c);
                                if(t<0||t>=colorn){
                                    MessageBox(GetHWnd(),"颜色不存在！","错误",MB_ICONERROR);
                                }else if(t==bkci){
...
int checkPattern(int x,int y,int player){
    int maxLevel=0,dirs[4][2]={{1,0},{0,1},{1,1},{1,-1}};
...
void refresh(){
    bkcolor=colorl[bkci];
    linecolor=colorl[lci];
    textcolor=colorl[tci];
    timecolor=colorl[timeci];
    setbkcolor(bkcolor);
    setlinecolor(linecolor);
    settextcolor(textcolor);
}
void judges1(){
    if(s1==0){
        ...
        fillrectangle(760,285,840,325);
        setfillcolor(WHITE);
        fillrectangle(795,290,765,320);
    }else{
        setfillcolor(GREEN);
        fillrectangle(760,285,840,325);
        setfillcolor(WHITE);
