...
int a[15][15],f,wx,wy,hh,ohh,mode,t,time1,time2,time3=30,time_start,time_gstart,time_gt,bwin,wwin,bkcolor,textcolor,linecolor,timecolor,bkci=1,tci=0,lci=0,timeci=0,colorl[10]={0,16777215,2763429,36095,55295,5737262,16760576,12500670,21930,13456052},dirs[4][2]={{1,0},{0,1},{1,1},{1,-1}};
char c[50];
bool g(int x,int y,int t),sjudge(int t),s1=1,s2=1,windowHidden_forever=1;
POINT p;
string key;
ofstream fout;
ifstream fin;
    ...
    while(true){
        ...
        if(KEY_DOWN(L_MOUSE)){
            ...
            if(sjudge(1)){
                ...
                while(true){
                    if(KEY_DOWN(L_MOUSE)){
                        ...
                        if(p.x>=20&&p.x<=70&&p.y>=5&&p.y<=45){
                            ...
                            MessageBox(GetHWnd()," AI 模式现处于测试阶段，出现问题可联系开发者，敬请谅解。"," AI 模式",MB_OK|MB_ICONINFORMATION);
                            InputBox(c,45,"请输入有效的deepseek api密钥的值"," AI 模式",NULL,0,0,0);
                            if(check_key(c)){
                                key=c;
                                mode=AI;
                                time_gstart=time(0);
                ...
                outtextxy(595,536,"恢复自定义设置");
                roundrect(590,530,840,580,30,30);
                outtextxy(627,616,"恢复默认设置");
                roundrect(620,610,840,660,30,30);
                outtextxy(57,536,"保存为自定义设置");
                roundrect(50,530,340,580,30,30);
                itoa(time3,c,10);
                st=c;
                if(time3<10){
                    st+=" s";
                ...
                while(true){
                    if(KEY_DOWN(L_MOUSE)){
                        ...
                        if(p.x>=20&&p.x<=70&&p.y>=5&&p.y<=45){
                            goto code_s;
                        }else if(p.x>=620&&p.x<=840&&p.y>=530&&p.y<=580){
                            t=1;
                            if(s2){
                            ...
                            if(t==1){
                                ...
                                fin>>s1>>s2>>time3>>bkci>>tci>>lci>>timeci>>bkcolor>>textcolor>>linecolor>>timecolor;
                                fin.close();
                                refresh();
                                MessageBox(GetHWnd(),"已成功恢复自定义设置","恢复自定义设置",MB_OK);
                                goto code_sy;
                            }
                        }else if(p.x>=50&&p.x<=303&&p.y>=530&&p.y<=580){
                            MessageBox(GetHWnd(),"已成功保存为自定义设置","保存为自定义设置",MB_OK);
                            fout.open("settings.txt");
                            fout<<s1<<' '<<s2<<' '<<time3<<' '<<bkci<<' '<<tci<<' '<<lci<<' '<<timeci<<' '<<bkcolor<<' '<<textcolor<<' '<<linecolor<<' '<<timecolor;
                            fout.close();
                        }else if(p.x>=620&&p.x<=840&&p.y>=610&&p.y<=660){
                            t=1;
                            if(s2){
                                t=MessageBox(GetHWnd(),"确定要恢复默认设置吗？\n\n（注：可在游戏设置中\"显示操作执行确认框\"关闭）","恢复默认设置",MB_ICONQUESTION|MB_OKCANCEL);
                            }
                            if(t==1){
                                s1=1,s2=1,time3=30,bkci=1,tci=0,lci=0,timeci=0;                                refresh();
...
xy white(){
    if(mode==classic){
        ...
        for(int i=0;i<cc.size();i++){
            st+='(';
            st+=itoa(cc[i].x+1,c,10);
            st+=',';
            st+=itoa(cc[i].y+1,c,10);
            st+=')';
        }
        st=call_ai(key,"一个棋盘有15行15列的格点，下五子棋，现给出一个由若干个整数数对组成的列表，依次表示棋盘上已有的棋子的行和列，列表奇数项是黑子，偶数项是白子，要求你给出列表下一项（帮那一方落子）。请严格按照x,y的格式返回答案(不要括号)，不要有思考过程。列表:"+st);
        if(st[0]=='E'){
            string st2;
            st2=" AI 模式中出现错误！错误编码:";
            st2+=st[1];
