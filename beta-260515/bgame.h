...
/*针对winodws 11新版命令提示符会自动弹出的问题（其它系统都不会），
特地编写以下2个函数以在程序运行时关闭命令提示符窗口*/
std::string st = "\\main";
...
//deepseek api密钥检测函数（原理为查询该密钥的余额）（deepseek辅助生成）
...
/*由于EasyX库仅支持GB-2312编码，而向deepseek api发送请求需要UTF-8编码，
所以本项目在main.cpp中采用GB-2312编码，在bgame.h中采用UTF-8编码，
某些函数调用时要先经以下2个函数进行编码转换*/

//GB2312转UTF-8编码转换函数（deepseek辅助生成）
...
//UTF-8转GB2312编码转换函数（deepseek辅助生成）
...
std::string call_ai(std::string key,std::string content){
    ...
    //3.构建JSON请求
    std::string json_data="{\"model\":\"deepseek-v4-flash\",\"messages\":[{\"role\":\"user\",\"content\":\""+escaped_content+"\"}],\"stream\":false,\"max_tokens\":2048}";
    //4.保存到临时文件
