...
int a[15][15],f,wx,wy,hh,ohh,mode,PID,time1,time2,time3=30,s1=1,s2=1,time_start,time_gstart,time_gt,bwin,wwin,bkcolor,textcolor,linecolor,timecolor,bkci=1,tci=0,lci=0,timeci=0,colorn=10,colorl[]={0,WHITE,RGB(165,42,42),RGB(255,140,0),RGB(255,215,0),RGB(46,139,87),RGB(0,191,255),RGB(190,190,190),BROWN,RGB(180,82,205)},dirs[4][2]={{1,0},{0,1},{1,1},{1,-1}};
char c[30];
...
void start(),sfill(int t),drawb(int lx,int rx,int y,string str),refresh(),judges1(),judges2(),runexe(),kill();
int main(){
    initgraph(900,800);
    move_window();
    time_start=time(0);
...
void start(){
    ...
    while(true){
        getmouse(&p);
        if(windowHidden_forever)windowHidden_forever=HideWindowByPartialTitle("\\main");
        if(sjudge(1)){
            sfill(1);
            setfillcolor(((!linecolor||lci==6)?linecolor+1:linecolor-1));
        ...
        if(ttt==1&&!sjudge(1)){
            ...
            floodfill(655,200,linecolor);
            ttt=0;
        }else if(ttt==2&&!sjudge(2)){
            setfillcolor(bkcolor);
            floodfill(225,350,linecolor);
            floodfill(655,350,linecolor);
            ttt=0;
        }else if(ttt==3&&!sjudge(3)){
            setfillcolor(bkcolor);
            floodfill(225,500,linecolor);
            floodfill(655,500,linecolor);
            ttt=0;
        }else if(ttt==4&&!sjudge(4)){
            setfillcolor(bkcolor);
            floodfill(225,650,linecolor);
            floodfill(655,650,linecolor);
            ttt=0;
