...
int a[15][15],f,wx,wy,hh,ohh,mode,t,time1,time2,time3=30,time_start,time_gstart,time_gt,bwin,wwin,bkcolor,textcolor,linecolor,timecolor,bkci=1,tci=0,lci=0,timeci=5,colorl[10]={0,16777215,2763429,36095,55295,5737262,16760576,12500670,21930,13456052},dirs[4][2]={{1,0},{0,1},{1,1},{1,-1}};
char c[50],ct;
bool g(int x,int y,int t),sjudge(int t),s1=1,s2=1,windowHidden_forever=1,is_keymr=0;
POINT p;
...
xy white(){
    if(mode==classic){
        ...
        st=call_ai(key,"你是一个五子棋大师。棋盘上有15行15列的格点，现给出一个由奇数个整数数对组成的列表，依次表示棋盘上已有的棋子的行和列，奇数项是黑子，偶数项是白子，现在让你来下白子，给出列表下一项。请严格按照x,y的格式返回答案，不要括号，不要有思考过程。列表:"+st);
        if(st[0]=='E'){
            st2=" AI 模式中出现错误！\n错误编码:";
            st2+=st.substr(1,2);
            st2+="\n错误描述:";
            st2+=utf8_to_gb2312(st.substr(3));
            st2+="\n\n将自动切换至经典模式。";
